...
<div id="context">
   ...
   <script type="text/javascript">
        ...
        (
        {
        url: '<?php echo base_url()?>moderator/berita/getList',
        dataType: 'json',
        colModel : [
            {display: 'ID', name : 'id', width : 40, sortable : true, align: 'center'},
            {display: 'Judul', name : 'judul', width : 100, sortable : true, align: 'left'},
            {display: 'Isi', name : 'isi', width : 250, sortable : true, align: 'left'},
            {display: 'Status', name : 'status', width : 100, sortable : true, align: 'left'},
            {display: 'Postdate', name : 'postdate', width : 200, sortable : true, align: 'left'},
            ],
        buttons : [
            ...
            if (com=='Delete'){
                if($('.trSelected',grid).length>0){
                    ...
                    $.ajax({
                       type: "POST",
                       dataType: "json",
                       url: "<?php echo base_url()?>moderator/berita/deleteItem",
                       data: {item : itemlist},
                ...
                window.open('<?php echo base_url()?>moderator/berita/addItem','_parent');
            }	
            else if (com=='Edit'){
                if(($('.trSelected',grid).length>0)&&($('.trSelected',grid).length<2)){
                    ...
                    window.open('<?php echo base_url()?>moderator/berita/editItem/'+itemlist, '_parent');
                }
            }
        }
    </script>
